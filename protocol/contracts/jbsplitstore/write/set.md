...
*   Loop through each `_currentSplits` to make sure the new `_splits` being set respect any current split bound by a lock constraint.
...
*   Check to make sure the provided `_splits` includes any locked current splits.
    ...
    ```
*   After the loop, delete the current splits from storage so we can repopulate them.
